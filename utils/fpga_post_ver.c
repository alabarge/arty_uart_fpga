...
int main(int argc, char *argv[]) {
   ...
   printf("\nfpga_version.h File Creation 1.4, Post Synthesis\n");
   ...
   if (argc < 5) {
      ...
      printf("usage : fpga_pre_ver fpga_checksum.txt fpga_crc32.txt fpga_md5.txt fpga_build.h fpga_version.h\n");
      return -1;
   }
   ...
   //
   sprintf(file_name, "%02X.%d.%d.%08X.%08X.bit", build_pid, build_map, build_logic, checksum, crc32);
   sprintf(in_line, "echo f|xcopy .\\output_files\\arty_top.bit .\\output_files\\arty_top_v%s /F /Y /R", file_name);
   printf("%s\n", in_line);
