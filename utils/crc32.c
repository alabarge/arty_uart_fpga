...
int main(int argc, char **argv) {
	...
	cval_not = (uint32_t)cval;

	printf("crc32 = %08X(LE), not crc32 = %08X, bytes = %d\n", cval, ~cval_not, clen);
	printf("crc32 = %08X(reverse), not crc32 = %08X\n", reverse_bits(cval), reverse_bits(~cval_not));
...
//    while ((nr = read(fd, buf, 4)) > 0)
//       nr = read(fd, buf, 4);
//       nr=4; buf[0]=0x84; buf[1]=0xE7; buf[2]=0x14; buf[3]=0x00;
//       nr=4; buf[3]=0x84; buf[2]=0xE7; buf[1]=0x14; buf[0]=0x00;
